...
export function positionsAt(match:GeneratedMatch,time:number){const state=stateAt(match,time),event=state.currentEvent,result:Record<string,Coordinate>={}; for(const [ti,team] of [match.definition.home,match.definition.away].entries()){const template=FORMATIONS[team.formation]??FORMATIONS["4-2-3-1"],ids=state.active[team.id]; ids.forEach((id,i)=>{const base=template[Math.min(i,template.length-1)],away=ti===1,phase=Math.sin(time/3.8+i*1.7),ballShift=((event.end?.x??52.5)-52.5)*.17; result[id]={x:clamp((away?105-base.x:base.x)+ballShift+(i?phase*1.65:0),2,103),y:clamp((away?68-base.y:base.y)+Math.cos(time/3.2+i)*1.3,2,66)};});} const elapsed=Math.max(0,time-at(event)),distance=Math.hypot((event.end?.x??52.5)-(event.start?.x??52.5),(event.end?.y??34)-(event.start?.y??34)),duration=event.type==="shot"||event.type==="goal"?Math.max(.65,distance/34):event.type==="carry"?Math.max(2.4,distance/7):Math.max(.9,distance/22),raw=clamp(elapsed/duration,0,1),progress=1-Math.pow(1-raw,3); const start=event.start??{x:52.5,y:34},end=event.end??start; return {players:result,ball:{x:start.x+(end.x-start.x)*progress,y:start.y+(end.y-start.y)*progress},progress:raw,duration,distance};}
